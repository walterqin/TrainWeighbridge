...
void Profile::init(const QString &filename)
{
	...
	if (!db.open())
		warning(tr("无法加载配置文件 %1\n%2").arg(filename).arg(db.lastError().text()));
	if (db.tables().empty())
		warning(tr("错误的配置文件 %1").arg(filename));
	...
	QString dataFile("data.db");
	db = QSqlDatabase::addDatabase("QSQLITE", dataName);
	db.setDatabaseName(dataFile);
	if (!db.open())
		warning(tr("无法打开数据库文件 %1").arg(dataFile));
	if (db.tables().empty())
		warning(tr("错误的数据库文件 %1").arg(dataFile));

	QString qcFile("qc.db");
	db = QSqlDatabase::addDatabase("QSQLITE", qcName);
	db.setDatabaseName(qcFile);
	if (!db.open())
		warning(tr("无法打开数据库文件 %1").arg(qcFile));
	if (db.tables().empty())
		warning(tr("错误的数据库文件 %1").arg(qcFile));
